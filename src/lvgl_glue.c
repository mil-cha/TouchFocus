 ...
 *
 * Geometry (see the BSP README / board_p4.h): native portrait 480 x 800.
 * LVGL renders directly in panel coordinates; no PPA rotation or touch remap is
 * required. The BSP's double framebuffer is still used for tear-free swaps.
 */
...
#define LV_HOR              PANEL_NATIVE_W        /* portrait width  = 480 */
#define LV_VER              PANEL_NATIVE_H        /* portrait height = 800 */
#define LVGL_TICK_PERIOD_MS 2
#define LVGL_TASK_STACK     8192
#define LVGL_TASK_PRIO      2

/* Application hook. Returning true consumes the touch as a wake-only event. */
extern bool touchfocus_touch_activity(bool pressed);
...
{
    /* full_refresh=1 hands us the whole native portrait frame. Copy it into the
     * BSP's off-screen framebuffer, then atomically swap front/back buffers. */
    const int w = area->x2 - area->x1 + 1;
    const int h = area->y2 - area->y1 + 1;
    board_p4_flush_region(area->x1, area->y1, w, h,
...
static void touch_read_cb(lv_indev_drv_t *drv, lv_indev_data_t *data)
{
    ...
    if (touchfocus_touch_activity(pressed)) {
        data->state = LV_INDEV_STATE_RELEASED;
        return;
    }

    if (pressed) {
        /* GT911 already reports native portrait panel coordinates. */
...
bool lvgl_glue_start(bool run_service_task)
{
    ...
    /* 2. TWO FULL-SCREEN portrait (480x800) draw buffers in PSRAM: LVGL renders
    ...
    /* 3. Display driver in native portrait orientation. */
    lv_disp_drv_init(&s_disp_drv);
    s_disp_drv.hor_res      = LV_HOR;   /* 480 */
    s_disp_drv.ver_res      = LV_VER;   /* 800 */
    ...
    s_indev_drv.long_press_time = 900;
    ...
    s_started = true;
    ESP_LOGI(TAG, "lvgl_glue_start OK (LVGL native portrait %dx%d)",
             LV_HOR, LV_VER);
    return true;
}
